...
import com.auratech.launcher.config.ProviderConfig;
...
public class LauncherProvider extends ContentProvider {
	...
	public  void initMaxItemId(){//micheal add when updata the mid and add the widget error
		mOpenHelper.setInitMaxItemId();
    }
	public  void initMaxScreenId(){//micheal add when updata the mid and add the widget error
		mOpenHelper.setInitMaxScreenId();
	}
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        DatabaseHelper(Context context) {
            ...
            // the DB here
            if (mMaxItemId == -1) {
            	try {//micheal add for android.database.sqlite.SQLiteCantOpenDatabaseException: unknown error (code 14): Could not open database
            		mMaxItemId = initializeMaxItemId(getWritableDatabase());
				} catch (Exception e) {
					// TODO: handle exception
					UtilDBOption.getInstance(mContext).copyFileOrDir("launcher.db");
					File dbFile = mContext.getDatabasePath("/data/data/com.auratech.launcher/databases/launcher.db");
					if (dbFile.exists()) {
						mMaxItemId = initializeMaxItemId(getWritableDatabase());
					}else {
						UtilDBOption.getInstance(mContext).copyFileOrDir("launcher.db");
					}
					Log.e(TAG, "----------------Exception---mMaxItemId="+dbFile.exists()+"   mMaxItemId="+mMaxItemId+e);
				}
            }
            if (mMaxScreenId == -1) {
            	try {//micheal add for android.database.sqlite.SQLiteCantOpenDatabaseException: unknown error (code 14): Could not open database
					...
					// TODO: handle exception
					mMaxScreenId = 3;//micheal add
					Log.e(TAG, "---Exception---mMaxScreenId="+mMaxScreenId+e);
				}
            }
        }
		
		private  void setInitMaxItemId(){//micheal add when updata the mid and add the widget error
			mMaxItemId = initializeMaxItemId(getWritableDatabase());
			
        }
		private  void setInitMaxScreenId(){//micheal add when updata the mid and add the widget error
			mMaxScreenId = initializeMaxScreenId(getWritableDatabase());
